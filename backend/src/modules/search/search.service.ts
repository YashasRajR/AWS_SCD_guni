 ...
 * a global search bar must not leak attendee/payment data to a role
...
export const searchService = {
  async search(query: string, permissions: string[]): Promise<SearchResult[]> {
    ...
    if (permissions.includes(PERMISSIONS.MANAGE_PAYMENTS)) {
      tasks.push(searchRepository.payments(trimmed), searchRepository.invoices(trimmed));
    }
