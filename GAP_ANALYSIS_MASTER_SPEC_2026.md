...
| Registration ID format | ✅ Done | commit `ffa0bda` — a native Postgres sequence (`registration_number_seq`, `MAXVALUE 999 NO CYCLE`) generates `GUNI AWS SCD <YY> <NNN>` atomically inside the same INSERT as the registration row: concurrency-safe, never reused after cancellation (sequences don't roll back), and hard-capped at 999 (the DB itself raises once exhausted, mapped to a clean validation error). Added the missing UNIQUE constraint on registration_number too. |
| Admin — Attendee Management (spec #34) | ✅ Done | commits `5120ea8`/`ec993ed` — edit/archive/restore attendee profiles (the `deleted_at` column from migration 050 finally has an admin endpoint), `GET /admin/attendees/:id` aggregates registration/payment/ticket/invoice/QR tokens/checkpoint progress/audit history from the existing per-domain services (no duplicated data), "reset account access" revokes sessions and sends a password-reset email, and a new admin detail page surfaces all of it plus reissue/regenerate/resend/rotate/revoke actions against the existing tickets/invoices/qr-tokens endpoints. "Add" (manual attendee creation) intentionally skipped — would duplicate self-registration with no distinct admin need in the spec text. |
| Admin — Document Center (spec #35) | ✅ Done | commit `b00e51c` — the attendee-detail page from #34 now also shows document generation status, reissue history (reusing the existing /versions endpoints from #62), and per-document email delivery status (`emailsService.listForUser()`, new). "Revoke old ticket where necessary" was already covered by the existing reissue flow revoking the old QR pair. |
| Admin — Payment Management (spec #36) | ✅ Done | commit `f5f17a0` — PaymentProvider gained fetchOrderStatus()/refundPayment() (Razorpay REST calls); admin can now view transaction details + coupon, check live gateway status, retry verification for a payment whose webhook may have been lost, initiate a gateway refund, and record a manual reconciliation (PAID/FAILED/REFUNDED with a mandatory reason) -- all audit-logged with admin identity + timestamp via the existing audit_logs, so "never silently alter financial records" holds without a bespoke audit table. confirmPaid()/confirmFailed() were extracted from the webhook handler so these admin paths go through the identical confirm/notify/invoice-issue logic. Search/filter/export already existed. |
